...
class TinyImageNetDataset(Dataset):
    def __init__(self, root, ipc, mem=False, shuffle=False, transform=None):
        super(TinyImageNetDataset, self).__init__()
        self.root = root
        self.classes = [d for d in os.listdir(root) if os.path.isdir(os.path.join(root, d))]  # Get all class directories
        self.ipc = ipc
        self.mem = mem
    ...
    def _load_data(self):
        for c, class_name in enumerate(self.classes):
            ...
            if not os.path.exists(dir_path):
                ...
                continue
            file_ls = [f for f in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, f))]
            if self.shuffle:
                random.shuffle(file_ls)
            for i in range(min(self.ipc, len(file_ls))):
                image_path = os.path.join(dir_path, file_ls[i])
                self.image_paths.append(image_path)
                self.targets.append(c)
                if self.mem:
                    self.samples.append(self.loader(image_path))

    def loader(self, path):
        return torchvision.datasets.folder.default_loader(path)
