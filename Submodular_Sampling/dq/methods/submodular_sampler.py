...
class Submodular_sampler(EarlyTrain):
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200, specific_model=None, balance=False,
        ...
        super(Submodular_sampler, self).__init__(dst_train, args, fraction, random_seed, epochs, specific_model, **kwargs)
        ...
        with self.model.embedding_recorder:
            ...
            if self.balance:
                ...
                for c in range(self.num_classes):
                    for k in range(self.args.K):#kmeans cluster num
       
                        class_cluster_indices = np.load(os.path.join(self.args.cluster_path, f'class_{c}_cluster_{k}.npy'))

                        c_indx = class_cluster_indices

                        # Calculate gradients into a matrix
                        gradients = self.calc_gradient(index=c_indx)
                        # Instantiate a submodular function
                        budget = int(self.args.IPC / self.args.K ) if self.args.IPC != 0 else int(self.args.fraction /self.args.K)
                        if budget == 0:
                            budget = 1
                        ...
                        submod_optimizer = submodular_optimizer.__dict__[self._greedy](args=self.args,
                                            index=c_indx, budget=budget, already_selected=[])
                ...
                file_name = f'sample.npy'
                file_path = os.path.join(self.args.save_path, file_name)
                np.save(file_path, selection_result)
