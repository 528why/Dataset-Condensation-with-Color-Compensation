...
class NaiveGreedy(optimizer):
    ...
    def select(self, gain_function, update_state=None, **kwargs):
        ...
        for i in tqdm(range(sum(selected), self.budget)):
            ...
            #all_gains.extend([(gain, idx) for idx, gain in enumerate(greedy_gain) if not selected[idx]])#
        ...
        # Sort all gains in descending order and return the top `self.budget` indices
        # all_gains_sorted = sorted(all_gains, reverse=True, key=lambda x: x[0])
        # Get only the firt `self.budget` items from sorted gains
        #top_gains = all_gains_sorted[:self.budget]
        # Extract and return the corresponding indices
        # sorted_indices = [idx for _, idx in top_gains]
         #return self.index[sorted_indices]  #
